...
### 4.1 环境数据 (environment)

| 字段 | 单位 | 范围 | 说明 |
|------|------|------|------|
| temperature | °C | -40~85 | 温度 (AHT20+BMP280平均值) |
| humidity | %RH | 0~100 | 相对湿度 (AHT20) |
| pressure | Pa | 30000~110000 | 大气压强 (BMP280) |
| altitude | m | -500~9000 | 海拔高度 (估算值) |
...
class MedicineBoxClient {
    ...
    private fun handleCommandResponse(json: String) {
        ...
        if (result == "ok") {
            ...
            val errorMsg = response.optString("error_msg", "未知错误")
            println("命令 $cmd 执行失败: $errorMsg")
        }
    }
