...
| 23 | **宿主 UI 卡死 ≠ 连接断**（issue #5 实测）：3.2.149 上 `export.render format=png scope=page` **三连 30s 超时**，同一秒 `format=svg` 正常返回（166012 bytes）——同一调用路径只差 `fileType`；期间 `bridge status` `connected`、心跳正常、`routed 22`，编辑器自己的**进度条卡住**。**心跳连续不能用来排除宿主故障**：桥能读到的（socket/心跳/路由计数）都只描述*通道*，宿主一个永不 settle 的 promise 与一个空闲编辑器从外面看一模一样。0.4.20 起 `export.render` 的 TIMEOUT 文案**并列两个假设**（① 参数不被宿主接受——2026-09-18 的 .d.ts 坑；② PNG 光栅化路径卡死）并给出 `format=svg` 指引；新增 `timeoutMs`（clamp 1–60 s，缺省 30 s 不变）。checkup 画布阶段（031 §2）PNG 用 **10s** 短绳、超时后**同页回退一次 SVG**，entry 记 `format: "svg"` + `pngError`（原错误必须留痕，静默换格式 = 把宿主缺陷洗成"一切正常"）；**只有 TIMEOUT 触发回退**（BAD_REQUEST / NOT_IMPLEMENTED 是宿主在说话，回退只会掩盖真问题）。触发因（是否与打标 marker 有关）**未验证**，不许当结论 | 出图卡住时：先看调用自己的超时（这才是唯一诚实信号），再试 `format=svg`；checkup 已内置回退，`report.json` 的 `ai_slots.canvas_images[*]` 里 `format`/`pngError` 会写明哪条腿成立。出处 issue #5、`outputs/031_*.txt`、`docs/bridge.md` §10.28 |
