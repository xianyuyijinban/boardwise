...
| 23 | **宿主 UI 卡死 = 导出管线漏进度条 toast，与导出成败无关**（031/032，issue #5）：3.2.149 上 `export.render format=png scope=page` 曾**三连 30s 超时**而同一秒 `format=svg` 正常；但 2026-09-24 岳在同一台机器上实测**成功也卡**——`canvas-sch.png` 664172B 正常落盘（无回退、无 error），编辑器照样停在 99%，同日另一窗 326KB 成功 + 卡 99% 同形。所以卡死**不是超时症状、也不是失败症状**，是 ManufactureData 导出管线开了进度条却不退它（成功/超时都漏）。**0.4.21 起 connector 自己拆**：`export.render` 的调用区包 try/**finally**，finally 里 400ms 后best-effort 调 `sys_LoadingAndProgressBar.destroyProgressBar()` + `destroyLoading()`（各自 typeof 守卫 + try/catch，宿主没有这个命名空间或 destroy 抛错都不影响导出结果）——修法源自上游 easyeda-agent 的同款实锤。超时文案末句也随之改：不再让人 reload，而是说 connector ~0.4s 后会拆 toast，仍挂着才 reload。另外**心跳连续不能用来排除宿主故障**：桥能读到的（socket/心跳/路由计数）都只描述*通道*。031 的 **PNG→SVG 回退**与这件事是两码事：回退救的是"PNG 那条腿超时"（checkup 不再失败），**不阻止 UI 卡死**；两者叠加后 149 上 checkup 应既不败也不留疤。**149 上的视觉验收归岳**（本批未做） | 出图卡住时：先看调用自己的超时（唯一诚实信号），再试 `format=svg`；0.4.21 起 UI 上的卡死 toast 应 ~1 秒内自清；checkup 的 `report.json` 里 `ai_slots.canvas_images[*]` 的 `format`/`pngError` 写明哪条腿成立。出处 issue #5、`outputs/031_*.txt`、`outputs/032_*.txt`、`docs/bridge.md` §10.28 |
