...
预期：窗口标题栏里有版本号，`帮助 → 关于` 里能看到 **3.2.149 以上**的版本。

不对怎么办：版本低于 3.2.149 时 boardwise 往下不再作保证——它依赖的那几个画布接口（打标、缩放）
只在 3.2.149 及以上实测过，更低的版本没有任何证据（是没测过，不是判定为坏）。升级到最新版再来。
（3.2.149 本身**已实测支持**：2026-09-23 在 3.2.149.88089769 上，8 个成员级 probe 全是带 arity
的 `function`、冷启动 `activate()` 正常派发、`getExportDocumentFile` 实跑出 308 KB PNG——
`doctor` 因此把 3.2.149 判为达标，不再卡它。）
...
```text
  PASS 编辑器安装版本 ≥ 3.2.149（离线读安装目录，不用扩展）
...
| 行 | 在问什么 | 红了怎么办 |
|---|---|---|
| 编辑器安装版本（离线） | 装在本机的编辑器达不达标——不用扩展、不用 daemon，装完立创就能查 | 先升级编辑器到 ≥3.2.149，其余检查都排在它后面 |
...
| 编辑器版本 ≥ 3.2.149 | 正在跑的这个编辑器达不达标（经桥复核，与离线项互为印证） | 升级立创 EDA Pro；刚升级过就重启编辑器 |
