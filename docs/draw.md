...
PYTHONPATH=src python -m boardwise.cli bridge call --action sys.probe \
    --params '{"checks": true}'
#   -> checks: {<ns>: {present, checked, missing, status: {member: typeof}, notes?}}
...
#   failed" stay distinguishable. A method the package marks `ADD since EDA v…`
#   that reads back `undefined` gets a `notes` entry carrying the contradiction.
