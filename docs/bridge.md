...
| `sys.get_document_file` | connector | read | connector | `fileType`, `fileName`, `password`, `timeoutMs` | `{fileType, source, encoding: 'base64', name, mime, bytes, data, isZip, note?}` — the open document as an `.epro`/`.epro2` archive (025): the online half of the offline pipeline, so the bytes go through `load_epru_text → build_schematic_model` unchanged. The declaration gates this call on **工程设计图 > 文件导出** and says a missing grant throws *every time*; the connector cannot read grants, so a refusal carries the host's own message plus the documented gates in `detail.permissions`, and `isZip` is reported rather than assumed (an `.epro2` whose first two bytes are not `PK` will not parse) | 30 s |
